...
function DashboardBisnis() {
  ...
  return <div className="min-h-screen bg-[#f6f7fb] text-[#17182d]">
     <aside className="fixed inset-y-0 left-0 z-20 hidden w-[256px] border-t-2 border-[#199900] bg-[#292663] text-white lg:block">
       <div className="flex h-[120px] items-center gap-4 bg-[#199900] px-6"><span className="flex h-16 w-16 shrink-0 items-center justify-center rounded-full bg-white text-[#199900]"><Building2 className="h-8 w-8" /></span><div className="min-w-0"><p className="whitespace-nowrap text-[23px] font-medium">Rawamangun</p><p className="mt-2 whitespace-nowrap text-[18px] text-white/75">Bisnis</p></div></div>
       <nav className="pt-0 text-[14px]"><SideItem icon={<LayoutGrid />} label="Dashboard" /><button type="button" onClick={() => setTargetMenuOpen((open) => !open)} className="flex h-12 w-full items-center gap-4 border-l-4 border-white bg-[#3d35d9] px-8 text-left text-white hover:bg-[#4840e0]"><FileText className="h-6 w-6 shrink-0" /><span className="flex-1 whitespace-nowrap">Pencapaian Tim</span><ChevronDown className={`h-4 w-4 transition-transform ${targetMenuOpen ? "rotate-180" : ""}`} /></button>{targetMenuOpen && <div className="bg-[#211f58] py-1 text-white/90"><SideSubItem label="Target Aktivitas" active /><SideSubItem label="Pencapaian Kepala KCP" /><SideSubItem label="Pencapaian Penaksir" muted /><SideSubItem label="Pencapaian Sales Officer" muted /><SideSubItem label="Pencapaian Sales Agent" muted /></div>}<SideItem icon={<ClipboardCheck />} label="Pengajuan Event" /><SideItem icon={<FileText />} label="Realisasi Event" /><SideItem icon={<LogOut />} label="Pengembalian Realisasi" /><SideItem icon={<FileText />} label="Riwayat Event" /><div className="mt-12"><SideItem icon={<LayoutGrid />} label="Ubah Kata Sandi" /><SideItem icon={<FileText />} label="Keluar" /></div></nav>
    </aside>
...
function SideSubItem({ label, active = false, muted = false }: { label: string; active?: boolean; muted?: boolean }) { return <div className={`relative flex h-12 items-center pl-16 pr-8 text-[14px] whitespace-nowrap ${muted ? "text-white/60" : ""}`}>{active && <span className="absolute left-10 h-3 w-3 rounded-full bg-white" />}{label}</div>; }
