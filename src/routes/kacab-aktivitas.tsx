...
type KacabActivityItem = { id: string; kcp: string; date: string; time: string; title: string; activityTypes?: string[]; place: string; region: string; priority: string; photoUrl?: string };
const ITEMS: KacabActivityItem[] = [
   { id: "seed-monang", kcp: "MAS MONANG-MANING", date: "2026-06-28", time: "15:00 - 16:00 WIB", title: "Evaluasi Pencapaian Target Unit & Sales", activityTypes: ["Evaluasi Pencapaian Target Unit & Sales"], place: "Balai Desa Kelurahan Pilumpanua", region: "Wajo, Sulawesi Selatan", priority: "High" },
   { id: "seed-rawamangun", kcp: "MAS RAWAMANGUN", date: "2026-06-06", time: "09:00 - 10:00 WIB", title: "Visit Nasabah One Obligor", activityTypes: ["Visit Nasabah One Obligor"], place: "KCP Rawamangun", region: "Jakarta Timur, DKI Jakarta", priority: "Medium" },
];
...
function KacabActivityList() {
  ...
  const activityItems = useMemo(() => [...ITEMS, ...activities.map(toKacabActivityItem)], [activities]);
    ...
    <MobileShell role="kacab" hideFab>
      ...
      <main className="space-y-4 bg-white px-5 pb-8">
        ...
        <div className="space-y-4">
           {groups.map((group) => <section key={group.kcp} className="overflow-hidden rounded-2xl bg-[#eef5ff]"><h2 className="flex items-center gap-2 px-4 py-3 text-[16px] font-bold text-[#2953A4]"><Building2 className="h-5 w-5" />{group.kcp}</h2><div className="space-y-3 px-1 pb-1">{group.items.map((item) => { const key = item.id; return <article key={key} className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><p className="text-[14px] font-bold text-[#2953A4]">{formatDate(item.date)} | {item.time}</p><span className={`flex-shrink-0 text-[14px] font-semibold ${item.priority === "High" ? "text-red-500" : "text-amber-600"}`}>{item.priority}</span></div><div className="my-3 border-t border-slate-100" /><h3 className="text-[18px] font-bold leading-tight text-slate-950">{formatActivityTitle(item.title, item.activityTypes)}</h3>{item.activityTypes && item.activityTypes.length > 1 && <p className="mt-1 text-[13px] font-medium text-[#2953A4]">{item.activityTypes.length} kegiatan dipilih</p>}<div className="mt-4 flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-3"><span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#eef5ff] text-[#2953A4]"><MapPin className="h-5 w-5" /></span><div className="min-w-0"><p className="truncate text-[15px] font-medium text-slate-900">{item.place}</p><p className="mt-0.5 text-[14px] text-slate-500">{item.region}</p></div></div><button type="button" onClick={() => setViewerItem(key)} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#315bac] py-3 text-[15px] font-semibold text-white"><Eye className="h-5 w-5" /> Lihat Foto</button></article>; })}</div></section>)}
       ...
       {viewerItem && <OverlayPortal><div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"><div className="w-full max-w-[360px] rounded-2xl bg-white px-5 pb-8 pt-7 shadow-2xl"><div className="flex items-center justify-between"><h2 className="text-[24px] font-bold text-slate-950">Foto Kegiatan</h2><button type="button" onClick={() => setViewerItem(null)} aria-label="Tutup"><X className="h-8 w-8 text-slate-950" /></button></div><div className="mt-6 flex h-[390px] items-center justify-center overflow-hidden rounded-xl bg-[#eeeeee] p-3">{(photos[viewerItem] || activityItems.find((item) => item.id === viewerItem)?.photoUrl) ? <img src={photos[viewerItem] || activityItems.find((item) => item.id === viewerItem)?.photoUrl} alt="Foto kegiatan" className="max-h-full max-w-full rounded-lg object-contain" /> : <p className="text-[14px] text-slate-500">Foto kegiatan belum tersedia</p>}</div><button type="button" onClick={() => { setCameraItem(viewerItem); setViewerItem(null); }} className="mx-auto mt-6 inline-flex items-center gap-2 rounded-xl bg-[#315bac] px-6 py-3 text-[15px] font-semibold text-white"><Camera className="h-5 w-5" /> Foto Ulang</button></div></div></OverlayPortal>}
...
function formatActivityTitle(fallback: string, activityTypes?: string[]) {
  ...
  return activityTypes.length === 1 ? activityTypes[0] : `${activityTypes[0]} +${activityTypes.length - 1} kegiatan`;
}
