...
function KacabActivityList() {
  ...
  return (
    <MobileShell role="kacab" hideFab>
      ...
      {viewerItem && <OverlayPortal><div className="fixed inset-0 z-50 bg-[#d7d7d7] px-5 pb-8 pt-8"><div className="mx-auto flex h-full max-w-[440px] flex-col"><div className="flex items-center justify-between"><h2 className="text-[26px] font-bold text-slate-950">Foto Kegiatan</h2><button type="button" onClick={() => setViewerItem(null)} aria-label="Tutup"><X className="h-8 w-8 text-slate-950" /></button></div><div className="mt-5 flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-2xl border-2 border-white/70 bg-[#eeeeee] p-3">{photos[viewerItem] ? <img src={photos[viewerItem]} alt="Foto kegiatan" className="max-h-full max-w-full rounded-xl object-contain" /> : <p className="text-[14px] text-slate-500">Foto kegiatan belum tersedia</p>}</div><button type="button" onClick={() => { setCameraItem(viewerItem); setViewerItem(null); }} className="mx-auto mt-5 inline-flex items-center gap-2 rounded-xl bg-[#315bac] px-6 py-3 text-[15px] font-semibold text-white"><Camera className="h-5 w-5" /> Foto Ulang</button></div></div></OverlayPortal>}
