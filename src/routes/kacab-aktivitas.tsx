    ...
    <MobileShell role="kacab" hideFab>
      ...
      <main className="space-y-4 bg-white px-5 pb-8">
        ...
        <div className="space-y-4">
          {groups.map((group) => <section key={group.kcp} className="overflow-hidden rounded-2xl bg-[#eef5ff]"><h2 className="flex items-center gap-2 px-4 py-3 text-[16px] font-bold text-[#2953A4]"><Building2 className="h-5 w-5" />{group.kcp}</h2><div className="space-y-3 px-1 pb-1">{group.items.map((item) => { const key = `${item.kcp}-${item.date}`; const hasPhoto = Boolean(photos[key]); return <article key={key} className="rounded-xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><p className="text-[14px] font-bold text-[#2953A4]">{formatDate(item.date)} | {item.time}</p><span className={`flex-shrink-0 text-[14px] font-semibold ${item.priority === "High" ? "text-red-500" : "text-amber-600"}`}>{item.priority}</span></div><div className="my-3 border-t border-slate-100" /><h3 className="text-[18px] font-bold leading-tight text-slate-950">{item.title}</h3><div className="mt-4 flex items-center gap-3 rounded-xl border border-slate-200 px-3 py-3"><span className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-[#eef5ff] text-[#2953A4]"><MapPin className="h-5 w-5" /></span><div className="min-w-0"><p className="truncate text-[15px] font-medium text-slate-900">{item.place}</p><p className="mt-0.5 text-[14px] text-slate-500">{item.region}</p></div></div><button type="button" onClick={() => hasPhoto ? setViewerItem(key) : setCameraItem(key)} className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-[#315bac] py-3 text-[15px] font-semibold text-white">{hasPhoto ? <Eye className="h-5 w-5" /> : <Camera className="h-5 w-5" />} {hasPhoto ? "Lihat Foto" : "Ambil Foto"}</button></article>; })}</div></section>)}
      ...
      {viewerItem && <OverlayPortal><div className="fixed inset-0 z-50 bg-[#d7d7d7] px-5 pb-8 pt-8"><div className="mx-auto flex h-full max-w-[440px] flex-col"><div className="flex items-center justify-between"><h2 className="text-[26px] font-bold text-slate-950">Foto Kegiatan</h2><button type="button" onClick={() => setViewerItem(null)} aria-label="Tutup"><X className="h-8 w-8 text-slate-950" /></button></div><div className="mt-5 flex min-h-0 flex-1 items-center justify-center overflow-hidden rounded-2xl border-2 border-white/70 bg-[#eeeeee] p-3"><img src={photos[viewerItem]} alt="Foto kegiatan" className="max-h-full max-w-full rounded-xl object-contain" /></div><button type="button" onClick={() => { setCameraItem(viewerItem); setViewerItem(null); }} className="mx-auto mt-5 inline-flex items-center gap-2 rounded-xl bg-[#315bac] px-6 py-3 text-[15px] font-semibold text-white"><Camera className="h-5 w-5" /> Foto Ulang</button></div></div></OverlayPortal>}
