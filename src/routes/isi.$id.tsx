...
import { activities, profile } from "@/lib/mock-data";
...
export const Route = createFileRoute("/isi/$id")({
  ...
  loader: ({ params }) => {
    ...
    if (!a) throw notFound();
    if (a.mode !== "online") throw notFound();
    return a;
  },
});
...
function PublicLeadForm() {
    ...
    return (
      <MobileShell hideNav>
        <div className="flex min-h-[80vh] flex-col items-center justify-center px-8 text-center">
          ...
          <h1 className="mt-4 text-lg font-bold">Terima kasih, {name.split(" ")[0] || "Kak"}!</h1>
          <p className="mt-2 text-sm text-muted-foreground">
            Data kamu sudah masuk ke tim <b>{profile.name}</b>. Kami hubungi via WA maksimal
            1x24 jam untuk info gadai emas.
          </p>
          <p className="mt-3 rounded-full bg-secondary px-3 py-1 text-[11px] text-muted-foreground">
  ...
  return (
    <MobileShell hideNav>
      <header className="px-5 pb-6 pt-12 text-brand-foreground" style={{ background: "var(--gradient-brand)" }}>
        <p className="inline-flex items-center gap-1.5 rounded-full bg-white/15 px-2.5 py-1 text-[11px] font-medium">
          <Globe className="h-3.5 w-3.5" /> Formulir Online · Gadai Mas
        </p>
