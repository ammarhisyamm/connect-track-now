...
function EditModal({ edit, setEdit, onSave }: { edit: EditTarget; setEdit: (value: EditTarget | null) => void; onSave: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/35 p-4"><div className="relative h-[371px] max-h-[calc(100vh-32px)] w-full max-w-[500px] overflow-hidden rounded-lg bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-200 px-8 py-5"><h2 className="text-[24px] font-bold">Edit Target Aktivitas</h2><button onClick={() => setEdit(null)} aria-label="Tutup"><X className="h-6 w-6 text-slate-500" /></button></div><div className="space-y-3 overflow-y-auto px-8 py-5 pb-20"><div className="grid gap-3 md:grid-cols-2"><Input label="Total Activity" required value={edit.total} onChange={(value) => setEdit({ ...edit, total: value })} /><Input label="Weekly Activity" required value={edit.weekly} onChange={(value) => setEdit({ ...edit, weekly: value })} /></div><Input label="Daily Activity" required value={edit.daily} onChange={(value) => setEdit({ ...edit, daily: value })} /><div><p className="text-[16px] font-medium text-slate-700">Status<span className="text-red-500">*</span></p><div className="mt-1.5 grid gap-3 md:grid-cols-2"><button type="button" onClick={() => setEdit({ ...edit, active: true })} className={`flex items-center gap-2 rounded-lg border px-4 py-2.5 text-left text-[14px] ${edit.active ? "border-2 border-[#199900] text-[#199900]" : "border-slate-200 text-slate-600"}`}><span className={`h-5 w-5 rounded-full border-2 ${edit.active ? "border-[#199900]" : "border-slate-400"}`}>{edit.active && <span className="mx-auto mt-0.5 block h-2.5 w-2.5 rounded-full bg-[#199900]" />}</span>Active</button><button type="button" onClick={() => setEdit({ ...edit, active: false })} className={`flex items-center gap-2 rounded-lg border px-4 py-2.5 text-left text-[14px] ${!edit.active ? "border-2 border-slate-500 text-slate-700" : "border-slate-200 text-slate-600"}`}><span className={`h-5 w-5 rounded-full border-2 ${!edit.active ? "border-slate-500" : "border-slate-400"}`}>{!edit.active && <span className="mx-auto mt-0.5 block h-2.5 w-2.5 rounded-full bg-slate-500" />}</span>Inactive</button></div></div></div><div className="absolute bottom-0 left-0 right-0 flex justify-end gap-3 border-t border-slate-200 bg-white px-8 py-4"><button onClick={() => setEdit(null)} className="rounded-lg border-2 border-[#199900] px-5 py-2.5 text-[14px] font-medium text-[#199900]">Batalkan</button><button onClick={() => setConfirming(true)} className="rounded-lg bg-[#199900] px-6 py-2.5 text-[14px] font-semibold text-white">Simpan</button></div>{confirming && <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/30 p-5"><div className="w-full max-w-[360px] rounded-xl bg-white p-5 shadow-xl"><h3 className="text-[17px] font-bold">Simpan perubahan target?</h3><p className="mt-2 text-[13px] leading-5 text-slate-500">Target aktivitas {edit.unit} akan diperbarui. Pastikan angka dan status sudah sesuai.</p><div className="mt-5 flex justify-end gap-3"><button onClick={() => setConfirming(false)} className="rounded-lg border border-slate-200 px-3 py-2 text-[13px]">Kembali Edit</button><button onClick={onSave} className="rounded-lg bg-[#199900] px-3 py-2 text-[13px] font-semibold text-white">Ya, Simpan</button></div></div></div>}</div></div>;
}
